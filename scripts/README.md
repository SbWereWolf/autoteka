...
`swap-env.ps1` нормализует `AUTOTEKA_ROOT` и `INFRA_ROOT` по текущей
ОС запуска: на Windows приводит WSL-пути `/mnt/c/...` к `C:\...`, а
в nix-окружении переводит Windows-пути вида `C:\...` в
