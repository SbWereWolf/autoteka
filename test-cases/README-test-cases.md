...
- Утверждение: шаблон `frontend/example.env` существует.
- Проверка: проверить наличие файла.
- Ожидаемый результат: шаблон присутствует в репозитории.
- Тип: automated
- Каталог тестов: `system-tests`
...
- Утверждение: server-side env-файлы для deploy:
  `/etc/autoteka/options.env` и файл по `TELEGRAM_ENV_FILE`.
- Проверка:
