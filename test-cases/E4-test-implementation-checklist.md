...
| TC-README-004 | frontend/tests/TC-README-004.spec.ts, frontend/e2e/TC-README-004.spec.ts | `cd frontend && npm test -- tests/TC-README-004` + `npm run test:e2e -- e2e/TC-README-004` |
...
Требуется `concurrently` (есть в backend или установить глобально):

```bash
npx concurrently -k --names "system,deploy,frontend,backend" \
  "cd system-tests && npm test" \
  "cd deploy/tests && npm test" \
  "cd frontend && npm test -- --exclude tests/TC-README-005" \
  "cd backend && php artisan test --filter=TcReadme --filter=TcBackend --filter=TcDeploy"
```

PowerShell (из корня):

```powershell
npx concurrently -k --names "system,deploy,frontend,backend" "cd system-tests; npm test" "cd deploy/tests; npm test" "cd frontend; npm test -- --exclude tests/TC-README-005" "cd backend; php artisan test --filter=TcReadme --filter=TcBackend --filter=TcDeploy"
```
...
```powershell
cd system-tests; npm test; cd ..\deploy\tests; npm test; cd ..\..\frontend; npm test -- --exclude tests/TC-README-005; cd ..\backend; php artisan test --filter=TcReadme --filter=TcBackend --filter=TcDeploy
```
...
# frontend (unit, без TC-README-005 для быстрого прогона)
cd frontend && npm test -- --exclude tests/TC-README-005

# frontend e2e (требует Docker)
cd frontend && npm run test:e2e

# backend (только Э4-тесты)
cd backend && php artisan test --filter=TcReadme --filter=TcBackend --filter=TcDeploy
```
...
|-----------|----------------------|-------------------|
| TC-README-010 | Server-side env `/etc/autoteka/deploy.env`, `telegram.env` | Требуется целевой сервер с установленным deploy |
