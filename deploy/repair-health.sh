...
restart_or_up() {
  ...
  if container_exists "$container"; then
    ...
    run_cmd /usr/bin/docker compose -f "$AUTOTEKA_ROOT/deploy/docker-compose.yml" up -d --build --remove-orphans "$service"
  fi
}

repair_backend_runtime() {
  run_cmd /usr/bin/docker compose -f "$AUTOTEKA_ROOT/deploy/docker-compose.yml" up -d --build --remove-orphans php
  ...
  if ! is_dry_run; then
    ...
    dry_run_log "/usr/bin/docker compose -f $AUTOTEKA_ROOT/deploy/docker-compose.yml up -d --build --remove-orphans web"
  fi
}
