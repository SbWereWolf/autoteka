...
envsubst '${CORS_ALLOW_ORIGIN} ${CORS_ALLOW_METHODS} ${CORS_ALLOW_HEADERS} ${CORS_EXPOSE_HEADERS} ${CORS_ALLOW_CREDENTIALS} ${CORS_MAX_AGE} ${API_CACHE_CONTROL_GET} ${API_CACHE_CONTROL_MUTATION}' < /etc/nginx/templates/default.conf.template > /etc/nginx/conf.d/default.conf
