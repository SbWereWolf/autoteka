...
12. если `frontend/.env` отсутствует, сборка web-контейнера создаёт его
    из `frontend/example.env`;
13. на host приводит runtime-каталоги Laravel к состоянию,
...
Если вместо изображения браузер получает `200 text/html` с `index.html`,
это означает, что запрос до `/storage/*` был обработан маршрутом SPA,
а не статической раздачей.
...
Production-контур использует same-origin запросы frontend -> `/api/v1`,
поэтому deploy nginx не должен требовать отдельного CORS-слоя.
...
`autoteka watch-changes` или `systemctl start autoteka-deploy.service`.
