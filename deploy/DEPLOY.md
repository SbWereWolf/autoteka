...
- `composer install` выполняется для `apps/API` и
  `apps/DatabaseOperator`;
- `SchemaDefinition` подключается как path-package и используется
  обоими приложениями;
- миграции и seed выполняются из `apps/DatabaseOperator`.
...
15. внутри `php` выполняет `composer install` для модулей `apps/API` и
    `apps/DatabaseOperator`, если для модуля есть `composer.json`;
...
18. создаёт `APP_KEY` через `ensure_app_key`, если ключа ещё нет, и
    синхронизирует его в `backend/.env`, `apps/API/.env`,
    `apps/DatabaseOperator/.env`;
...
Отдельные env-файлы приложений создаются в `apps/API/.env` и
`apps/DatabaseOperator/.env`:
...
### 5.5. deploy/bootstrap/bootstrap/config/deploy.example.env
...
- Backend в этом проекте включает 3 модуля, но runtime-логи Laravel
  пишутся в 2 приложения: `apps/API` и `apps/DatabaseOperator` (модуль
...
- Причина: `php artisan migrate --force` (из `apps/DatabaseOperator`)
  завершился ошибкой.
