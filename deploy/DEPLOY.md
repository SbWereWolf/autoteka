...
Актуальный deploy-алгоритм:

1. `deploy.sh` выполняет `git fetch`;
2. сравнивает локальный `HEAD` и `REMOTE/BRANCH`;
3. если commit совпадает, не меняет рабочую копию и завершает работу;
...
8. поднимает только контейнер `php`;
9. ждёт готовности контейнера к `docker compose exec` с timeout;
10. внутри `php` приводит runtime-каталоги Laravel к состоянию,
    пригодному для записи от `www-data`;
11. внутри `php` создаёт `.env`, если его нет;
12. внутри `php` выполняет `composer install`;
13. проверяет запуск `artisan`;
14. включает Laravel maintenance mode;
15. создаёт `APP_KEY`, если ключа ещё нет;
16. выполняет `php artisan migrate --force`;
17. выполняет `php artisan db:seed --class=AdminUserSeeder --force`;
18. снимает maintenance mode;
19. поднимает или перезапускает контейнер `web`;
20. отправляет success-уведомление в Telegram с hash и `subject`
    последнего раскатанного commit.

`deploy/php/entrypoint.sh` больше не выполняет миграции, сиды и
`composer install`: entrypoint отвечает только за runtime-start и
подготовку Laravel runtime-каталогов.
...
- `deploy.sh` всегда отправляет одно success-сообщение после успешной
  раскатки, для него lock-файл не создаётся;
...
- Исправление: устранить конфликт/проблему прав, затем вручную
  сохранить изменения и повторить deploy.

`DEPLOY_WORKTREE_DIRTY_AFTER_STASH`
...
`DEPLOY_RUNTIME_PERMISSIONS_FAILED`
...
- Проверьте: bind mount `backend`, readonly mount, владельца/права
  `storage`, `bootstrap/cache`, `database`, логи контейнера `php`.
...
remote, а при наличии расхождения сначала пытается очистить рабочую
копию через `git stash`, потом выполняет
