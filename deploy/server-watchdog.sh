...
run_domain_repair() {
  local domain="$1"
  local args=("$SCRIPT_DIR/repair-health.sh" "$domain")
...
if [ -n "$HOST_REASON" ]; then
  ...
  if [ -x "$SCRIPT_DIR/metrics-export.sh" ]; then
    if ! "$SCRIPT_DIR/metrics-export.sh" >/dev/null 2>&1; then
      ...
      notify_info "$SCRIPT_ID" "$WATCHDOG_ACTION" "WATCHDOG_METRICS_EXPORT_FAILED" "не удалось обновить deploy/metrics/data.json"
    fi
  fi
