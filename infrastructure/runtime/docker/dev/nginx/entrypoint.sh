...
export FRONTEND_UPSTREAM_HOST="${FRONTEND_UPSTREAM_HOST:-frontend}"
export FRONTEND_UPSTREAM_PORT="${FRONTEND_UPSTREAM_PORT:-${FRONTEND_PORT}}"
TEMPLATE=/etc/nginx/templates/source.conf.template
if [ "${FRONTEND_MODE}" = "bundle-watch" ]; then TEMPLATE=/etc/nginx/templates/bundle-watch.conf.template; fi
