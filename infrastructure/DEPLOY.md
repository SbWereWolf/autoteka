...
- `docs/manual/ADMIN_MANUAL.md` описывает **как с этим работать**:
  короткие практические команды и переходы в нужные разделы
  `DEPLOY.md`, без дублирования полного deploy-справочника.

`ADMIN_MANUAL.md` служит входной точкой для администратора, а этот
документ остаётся полной спецификацией deploy-скриптов, диагностики,
починки, backup и restore.
...
15. внутри `php` выполняет `composer install` для модулей
    `apps/ShopAPI` и `apps/ShopOperator`, если для модуля есть
    `composer.json`; локальный пакет
    `backend/packages/SchemaDefinition` в этом шаге не обрабатывается
    напрямую, потому что это path-пакет и его зависимости
    подтягиваются при установке зависимостей `ShopAPI` /
    `ShopOperator`.
...
19. выполняет `php artisan migrate --force` из `apps/ShopOperator`;
...
### 2.3. Карта deploy-скриптов и entrypoint-команд

Основной пользовательский интерфейс deploy-контура —
`/usr/local/bin/autoteka`. Он маршрутизирует команды в конкретные
shell-скрипты внутри `deploy/`.

#### Bootstrap

- `autoteka up` →
  `deploy/runtime/docker-compose.yml up -d --remove-orphans`
- `autoteka down` → `deploy/runtime/docker-compose.yml down`
- `deploy/bootstrap/install.sh` — первичная установка host-конфига,
  wrapper, systemd и логирования
- `deploy/bootstrap/uninstall.sh` или `autoteka uninstall <mode>` —
  controlled removal инсталляции

#### Runtime rollout

- `autoteka deploy` → `deploy/runtime/deploy.sh`
- `autoteka watch-changes` → `deploy/runtime/watch-changes.sh`

Использование:

- `deploy` — ручной повтор rollout для текущего локального `HEAD`
- `watch-changes` — manual trigger git polling и автодеплоя

Диагностика:

- `journalctl -u watch-changes.service -n 100 --no-pager`
- `/var/log/autoteka-deploy.log`
- [§7.4 Не работает автодеплой](#74-не-работает-автодеплой)

#### Repair и health

- `autoteka watchdog` →
  `deploy/observability/infrastructure/server-watchdog.sh`
- `autoteka repair-runtime` → `deploy/repair/repair-runtime.sh`
- `autoteka repair-health <domain>` → `deploy/repair/repair-health.sh`
- `autoteka health-reset <target>` → `deploy/repair/health-reset.sh`
- `autoteka repair-infra` → `deploy/repair/repair-infra.sh`
- `deploy/observability/application/metrics-export.sh` — формирование
  `/metrics/data.json`

Использование:

- `watchdog` — health-check и bounded auto-remediation
- `repair-runtime` — тяжёлая починка runtime с smoke-check `/up`,
  `/api/v1/category-list`, `/admin/login`
- `repair-health` — точечная починка одного домена:
  `nginx|php|backend|admin`
- `health-reset` — очистка health-state и Telegram dedup lock'ов
- `repair-infra` — восстановление таймеров и инфраструктурного
  состояния watchdog
...
- [§7 Диагностика поломок](#7-диагностика-поломок)

#### Maintenance, backup и restore

- `autoteka maintenance` → `deploy/maintenance/server-maintenance.sh`
- `autoteka backup-storage` → `deploy/maintenance/storage-backup.sh`
- `autoteka backup` → `deploy/maintenance/backup.sh`
- `autoteka restore <archive>` → `deploy/maintenance/restore.sh`

Использование:

- `maintenance` — периодическое техобслуживание
- `backup-storage` — архив `backend/storage` и `database.sqlite`
- `backup` — deploy-конфиг, env, systemd, docker, logrotate, fail2ban
- `restore` — восстановление deploy-конфига и очистка runtime
  health-state
...
- [§10 Резервное копирование и восстановление deploy-настроек](#10-резервное-копирование-и-восстановление-deploy-настроек)
...
- `HTTP_BIND_HOST` — bind host для production nginx publish (по
  умолчанию `127.0.0.1`);
...
Скрипты `deploy/maintenance/backup.sh` и
`deploy/maintenance/restore.sh` сохраняют и восстанавливают серверные
настройки, влияющие на работу приложения и Docker-сервисов:
