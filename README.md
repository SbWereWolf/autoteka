...
Текущая фаза работы над проектом — макетирование, 
есть один демонстрационный сервер, других серверов нет.
Разработка вся ведётся локально, чаще под WSL, чем под Windows.
Под Windows чаще ведётся ручное тестирование. Поэтому важно сохранять 
совместимость кода и конфигов через использование
[swap-env](scripts/swap-env.ps1)

DEV среда это локальная разработка

PROD среда это демонстрационный сервер на Ubuntu 24
