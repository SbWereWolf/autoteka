...
**Дата:** 2026‑03‑04  
**Проект:** клиентский макет каталога магазинов автозапчастей (Vue 3).  
**Цель документа:** описать, какие данные реально потребляет фронтенд, какие справочники и “общесайтовые” настройки существуют, и из каких объектов должен состоять бэкенд, чтобы быть **точной моделью** текущего фронта (с возможностью роста до зрелой системы).
...
- `src/state.ts` — глобальное состояние + сохранение выбора в `localStorage`
- `src/utils/sortShops.ts` — алгоритм сортировки магазинов

Чтобы заменить моки на API **без переписывания UI**, бэкенд должен отдавать:
- справочники (`cities`, `categories`, `features`, `defaultFeature`, опционально `themes`)
- список магазинов (в каталоге используется фильтр по городу + сортировка по выбранным категориям/фишке)
...
- **Магазины:** `src/mocks/shops.json` (импортируется как `src/mocks/shops`)
...
На чтении применяется “санитайзинг” (проверка, что значения существуют в справочниках), иначе используются fallback‑значения.
...
2) сортирует результат по правилам `sortShopsByRules({ selectedCategories, selectedFeature })`

3) выводит плитки `ShopTile` и счётчик `{{ sorted.length }} шт.`
...
- показывает галерею: берёт `galleryImages` если массив, иначе пустое состояние
...
Важно: совпадение по строкам, **без нормализации** (опечатки = разные значения).
...
**Рекомендация:** темы — это скорее *презентационный конфиг*, их можно:
- оставить фронту (CSS‑токены живут в `src/styles/themes.css`), или
- отдавать бэком как часть конфигурации приложения (если захотите централизованное управление / A/B).
...
- `email` → `mailto:`
- `telegram` и `whatsapp` → значение считается URL (открывается в новой вкладке, если начинается с `http`)
- `address` → просто текст (пока без карты)

Рекомендуемый серверный enum для валидации:

```ts
type ContactType = "phone" | "email" | "telegram" | "whatsapp" | "address" | "text";
```
...
2) Внутри каждой группы делим на “с выбранной фишкой / без”:
...
- перенести сортировку на сервер (но тогда контракт должен явно фиксировать правила и тесты).
...
- *(опционально)* `ThemeMeta` (как конфиг)
- *(опционально)* `ContactType` (enum/справочник для валидации)

### 7.3 Конфиг приложения (site‑level)
- `defaultFeature`
- *(опционально)* `defaultCityId` или `isDefault` в `City`
- *(опционально)* `themes[]` (если решите хранить централизованно)
...
- Таблица `shops` содержит `categories` и `features` как JSON‑массивы строк
...
API при этом всё равно может отдавать `categories: string[]` и `features: string[]` (по `name`), чтобы фронт не менять.
...
```json
{
  "cities": [{ "id": "barnaul", "name": "Барнаул", "isDefault": true }],
  "categories": ["Отечественные запчасти", "…"],
  "features": ["самая быстрая доставка", "…"],
  "defaultFeature": "самая быстрая доставка",
  "themes": [{ "id": "a-neutral", "label": "A • Neutral", "style": "A", "palette": "Neutral", "icon": "🌤️" }]
}
```
> `themes` можно не отдавать (оставить на фронте), если не нужно управлять темами с сервера.
...
- **Экономнее:** отдавать карточки `{ id, name, thumbUrl, categories, features, city }` и подгружать детали отдельно.
...
5) `shop.id` уникален
6) `shop.siteUrl` может быть пустым — UI корректно показывает “Сайт недоступен”
7) `galleryImages` может отсутствовать/быть пустым — UI показывает empty‑state
...
> Но при росте системы админка/CRUD логично появится: ресурсы `Shop`, `City`, `Category`, `Feature`, `Media`, `Contact` + роли “оператор/админ”.

---

## 12) Практичный next-step (если идти в реализацию)

1) Поднять BFF слой, который отдаёт `/api/dicts`, `/api/shops`, `/api/shops/:id`
2) Перевести фронт с импортов моков на fetch (без изменения компонентов)
3) Добавить серверную валидацию/тесты консистентности справочников ↔ магазинов
4) Только потом решать, нормализовать ли БД или оставить JSON‑поля на старте
