...
- раздаёт frontend и проксирует backend.
...
`vue-app-deploy.service` запускает `watch-changes.sh`, а не rollout
...
- `/etc/vue-app/deploy.env` — источник `AUTOTEKA_ROOT`, `BRANCH`,
  `REMOTE`, `HTTP_PORT`
- `/etc/vue-app/telegram.env` — optional Telegram secrets
