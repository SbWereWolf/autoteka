...
Агент обязан выполнять требования главы 
"1.1. Требования к разработке в исполнении LLM-агентом" буквально.
Запрещено ослаблять их, заменять «разумным эквивалентом», трактовать 
как советы, игнорировать частично или переставлять обязательные 
шаги местами, если документ прямо не допускает такого изменения порядка.
...
Агент обязан разрабатывать решения внутри этого стека.
...
- OOP объединяем данные и способы их обработки;
- DDD разделяем классы по зонам ответственности предметной области;
...
Агент обязан разрабатывать внутри этого стека.
Запрещено выходить за границы этого стека. Агент может на этапе
планирования предложить пользователю другое общепринятое решение за
границами этого стека с явным указанием на выход за границы.
