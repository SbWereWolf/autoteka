...
- `deploy/systemd/autoteka.service` — основной unit для запуска
  контейнеров через `docker compose up -d`
- `deploy/systemd/watch-changes.service` — unit для автодеплоя,
  запускает `watch-changes.sh`
- `deploy/systemd/watch-changes.timer` — timer для автодеплоя (каждые
  5 минут)
- `deploy/systemd/server-watchdog.service` — unit для
  watchdog-проверок
- `deploy/systemd/server-watchdog.timer` — timer для watchdog (каждые
  2 минуты)
- `deploy/systemd/server-maintenance.service` — unit для
  maintenance-операций
- `deploy/systemd/server-maintenance.timer` — timer для maintenance
  (ежедневно в 03:15)
